...
public class NetWorkUtils {
    ...
    public static boolean isNetConnect(Context context) {
        try {
            ...
            } else if (netWrokInfo.getTypeName().equalsIgnoreCase("MOBILE")// 修复友盟收集到的空指针，增加try
                    // catch
                    && StringUtils.isEmptyString(netWrokInfo.getExtraInfo())
                    && netWrokInfo.getExtraInfo().equalsIgnoreCase("cmwap")) {
                try {
                    Toast.makeText(
                            context,
                            "检测到当前接入点为CMWAP，建议进入[移动网络设置]-[>接入点]->选择[CMNET]类型",
...
//        CellInfo info =CellInfo.CREATOR. new CellInfo();
//        try {
